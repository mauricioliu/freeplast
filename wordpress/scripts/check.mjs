#!/usr/bin/env node
/**
 * Freeplast WordPress shell — automated acceptance checks (issues #2–#10, #12).
 *
...
test('every Product has a clean canonical URL, stays out of editor menus and renders v7-A from source-supported facts', async () => {
  ...
  assert.equal(wp(['option', 'get', 'fp_db_version']).stdout, String(DB_VERSION), `migration ${DB_VERSION} must be applied after activation (the sales workflow and notification slices bump it)`);
...
/* ─── 18. Write VERIFICATION.md and clean up ──────────────────────────── */

test('record mechanical proof in wordpress/VERIFICATION.md', () => {
  const lines = [
    `# Mechanical verification — Freeplast WordPress shell + catalog + discovery + quote basket + quote request + sales workflow + durable notifications + v6 content (issues #2–#10, #12)`,    ``,
  ...
  lines.push(
    ...
    `- The Quote Request submission (issue #8) is verified through served documents and the persisted fp_quote records: the manual Dirección de despacho is the this-slice address path (the Google-assisted confirmation arrives with issue #11). Human visual approval remains Gate 3.`,
    `- The sales administration workflow (issue #9) is verified through served wp-admin documents and the persisted fp_quote metadata: corrections, notes, status history and staff identity live on the records; Dispatch Distance retry (issue #11) arrives with its slice. Human visual approval remains Gate 3.`,
