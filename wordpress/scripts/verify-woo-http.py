...
card=re.search(r'<a href="[^"]*" class="woocommerce-LoopProduct-link woocommerce-loop-product__link">(?:(?!</a>).)*</a>',home,re.S)
assert card,'Home renders no native product link'
assert '</p>' not in card.group(0),'wpautop damage inside the product link'
assert re.search(r'<h2[^>]*>[^<]+</h2>',card.group(0)),'the product link does not carry its title'
